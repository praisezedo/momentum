    ...
    return (
        <>
         <span className= {`flex-center gap-5  border ${goal.completed ? "bg-gray-300 border-gray-500" : "bg-blue-300 border-blue-500"}  rounded-xl px-3 py-2`}> 
            <input type="checkbox" checked={goal.completed} className="w-5 hover:cursor-pointer h-5" onClick={ () =>  handleToggleGoal(goal.id)}/>
           <p> <strong className= {` text-xl ${goal.completed ? "text-gray-700" : "text-blue-900"} `} >•  {goal.title}</strong></p>
